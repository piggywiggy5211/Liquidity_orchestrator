...
def test_create_order_same_ts_different_body(client, clean_db):
    ...
    with patch("asyncio.sleep", AsyncMock()):
        # First request
        response1 = client.post("/orders", json={"direction": "on-ramp", "pair": "USDT-USD", "amount": 100.0, "incoming_account": "a", "outgoing_account": "b"}, headers={"X-Api-Ts": ts})
        assert response1.status_code == 200
        # Second request with different body
        response2 = client.post("/orders", json={"direction": "on-ramp", "pair": "USDT-USD", "amount": 200.0, "incoming_account": "a", "outgoing_account": "b"}, headers={"X-Api-Ts": ts})
        assert response2.status_code == 200
...
async def test_execute_order_parallel_conflict(session_factory):
    ...
    async with session_factory() as sess:
        ...
        service = LiquidityService(
            uow, PROVIDERS_MAP, InMemoryMetricsCollector(stats_window_seconds=60), Decimal("0.02")
        )
    ...
    async with session_factory() as sess1, session_factory() as sess2, session_factory() as sess3:
        ...
        with patch(
            ...
            service1 = LiquidityService(
                uow1, PROVIDERS_MAP, InMemoryMetricsCollector(stats_window_seconds=60), Decimal("0.02")
            )
            service2 = LiquidityService(
                uow2, PROVIDERS_MAP, InMemoryMetricsCollector(stats_window_seconds=60), Decimal("0.02")
            )
            service3 = LiquidityService(
                uow3, PROVIDERS_MAP, InMemoryMetricsCollector(stats_window_seconds=60), Decimal("0.02")
            )
